...
class SpeciesCacheService {
  ...
  Future<void> recordViewed(PerenualSpeciesSummary summary, PerenualSpeciesDetail detail) async {
    final prefs = await SharedPreferences.getInstance();
    final entries = _decode(prefs.getStringList(_prefsKey));
    entries.removeWhere((e) => (e['summary'] as Map)['id'] == summary.id);
    entries.insert(0, {'summary': _summaryToMap(summary), 'detail': _detailToMap(detail)});
  ...
  Future<List<RecentSpecies>> getRecent() async {
    ...
    return _decode(prefs.getStringList(_prefsKey)).map((entry) {
      return RecentSpecies(
        summary: PerenualSpeciesSummary.fromMap(Map<String, dynamic>.from(entry['summary'] as Map)),
        detail: PerenualSpeciesDetail.fromMap(Map<String, dynamic>.from(entry['detail'] as Map)),
      );
    }).toList();
  }

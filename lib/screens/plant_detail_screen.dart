      ...
      body: _loading
          ...
          : ListView(
              ...
              children: [
                ...
                const SizedBox(height: 20),
                Text('Watering History', style: TextStyle(fontWeight: FontWeight.w700, color: scheme.primary)),
                const SizedBox(height: 8),
                if (_careHistory.isEmpty)
                  Text('No watering history yet.', style: TextStyle(color: scheme.onSurfaceVariant))
                else
                  for (final entry in _careHistory)
                    Padding(
                      padding: const EdgeInsets.symmetric(vertical: 2),
                      child: Text(DateTime.parse(entry).toLocal().toString().split('.').first),
                    ),
              ],
            ),
    );
